...
class MYPDF extends TCPDF {
	...
	public function ColoredTable($header,$data) {
		...
		// Header
		$w = array(30,45,45,60,30,30,30,30,30);
		...
		$fill = 0;
		foreach($data as $row) {
			$this->Cell($w[0], 10, $row["id_pengguna"], 'LR', 0, 'C', $fill);
			$this->Cell($w[1], 10, $row["nama_depan"], 'LR', 0, 'L', $fill);
			$this->Cell($w[2], 10, $row["nama_belakang"], 'LR', 0, 'L', $fill);
            $this->Cell($w[3], 10, $row["email"], 'LR', 0, 'L', $fill);
            $this->Cell($w[4], 10, $row["no_telp"], 'LR', 0, 'L', $fill);
            $this->Cell($w[5], 10, "Rp " . number_format($row['e_money'], 2, ",", "."), 'LR', 0, 'C', $fill);
...
$pdf->Output('Detail Pengguna.pdf', 'I');
