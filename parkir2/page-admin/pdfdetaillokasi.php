...
class MYPDF extends TCPDF {
	...
	public function ColoredTable($header,$data) {
		...
		$fill = 0;
		foreach($data as $row) {
			$this->Cell($w[0], 10, $row["id_slot"], 'LR', 0, 'C', $fill);
			$this->Cell($w[1], 10, $row["id_lokasi"], 'LR', 0, 'C', $fill);
			$this->Cell($w[2], 10, $row["nama_slot"], 'LR', 0, 'L', $fill);
            $this->Cell($w[3], 10, $row["status_slot"], 'LR', 0, 'C', $fill);
            $this->Cell($w[4], 10, $row["lantai"], 'LR', 0, 'C', $fill);
...
$pdf->Output('Detail Detail Lokasi.pdf', 'I');
